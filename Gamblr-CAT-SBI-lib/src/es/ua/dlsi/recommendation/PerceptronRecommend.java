 ...
 * @author Miquel Esplà Gomis
 */
public class PerceptronRecommend {
    /**
     ...
     */
    public static int[] MakeRecommendation(Segment s, TranslationUnit tu,
        ...
        List<Pair<String,double[]>> features=feat.Compute(debugpw, false);
        if(debugpw!=null){
    ...
    public static int[] MakeRecommendationNM(Segment s, TranslationUnit tu,
                ...
                double decissionthreshold){
        int[] exit=new int[tu.getTarget().size()];
        MNFeatures mnfeat=new MNFeatures(s, tu, max_segmentlen, score);
    ...
    public static int[] MakeRecommendationNMNoNorm(Segment s, TranslationUnit tu,
        ...
        MNFeaturesNotNormalised mnfeatnotnorm=new
                MNFeaturesNotNormalised(s, tu, max_segmentlen,score);
     ...
     * @return Returns an array of integers with the recommendations: 1 means "keep untouched" and -1 means "change"
     */
    /*public static int[] MakeRecommendationNoFMS(Segment s, TranslationUnit tu,
        ...
        int[] exit=new int[tu.getTarget().size()];
        List<Pair<String,double[]>> features=FeaturesComputer.CalculateAllFeatures(s, tu,
                max_segmentlen,score,false, false);
        if(debug) {
            System.out.println("Segment: \""+tu.getTarget()+"\"");
        }
        for(int wpos=0;wpos<features.size();wpos++){
            ...
            double[] feature_values=feature.getSecond();
            double net=z;
            for(int j=0;j<feature_values.length-1;j++){
                net+=feature_values[j]*weights[j];
            }
            double value=(1.0/(1.0+Math.exp(-net)));
            exit[wpos]=PrintRecomendation(value, feature_values, feature.getFirst(),
