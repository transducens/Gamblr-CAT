 ...
 * @author miquel
 */
public class MNFeaturesNotNormalised extends FeaturesComputer {
    
    
    public MNFeaturesNotNormalised(Segment sourcesegment, TranslationUnit tu,
                    ...
                    for(Evidence evidence: tu.getEvidences()){
                        ...
                        if(Evidence.Cover(wpos, evidence.getTranslation().getPosition(),
                            ...
                            if(evidence.getSegment().getLength()==(mlen+1) &&
                                ...
                                if(matches) {
                                    coveringintersec_p.add((Segment)evidence.getTranslation());
                                    //coveringintersec_p.add(evidence);
                                }
                                else {
                                    coveringintersec_n.add((Segment)evidence.getTranslation());
                                    //coveringintersec_n.add(evidence);
                                }
                            }
                        }
                    }
                    if(!coveringintersec_p.isEmpty() || !coveringintersec_n.isEmpty()){
                        covering_evidence++;
                    }
                    //Positive source
                    feat_tmp[mlen*segmaxlen*2+nlen*2]=
                            (double)coveringintersec_p.size();
                    //Negative source
                    feat_tmp[mlen*segmaxlen*2+nlen*2+1]=
                            (double)coveringintersec_n.size();
                }
            }
            if(debugpw!=null){
